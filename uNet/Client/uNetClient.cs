...
namespace uNet.Client
{
    public class uNetClient : IDisposable
    {
        ...
        {
            lock (_sendLock) Processor.SendPacket(packet, NetStream).Wait();
        }
        ...
        private async Task<bool> ConnectAsync()
        {
            ...
            if (_uNetClient.Connected)
            {
                if (Settings is ClientSettings)
                {
                    ...
                    {
                        NetStream = new SslStream(_uNetClient.GetStream(), false, ValidateServerCertificate, null,
                                                   EncryptionPolicy.RequireEncryption);
                        await
                            (NetStream as SslStream).AuthenticateAsClientAsync(
                                (Settings as ClientSettings).SslServerCertIdentity.CertName);
                    }
                    else
                        NetStream = _uNetClient.GetStream();
                }
                else
                    NetStream = _uNetClient.GetStream();
                                                                 ...
                                                                 if (
                                                                     ...
                                                                     if(OnPacketReceived != null)
                                                                         OnPacketReceived(null,
                                                                             new PacketEventArgs(null, packet));
                                                             });
                }
